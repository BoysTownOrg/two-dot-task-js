...
function taskName(csvEntries) {
  return csvEntries[0].trim();
}

function parseCsvEntries(csvEntries, state, trials) {
  if (
    state.lastTaskName !== taskName(csvEntries) &&
    taskName(csvEntries) !== "Task"
  ) {
    if (
