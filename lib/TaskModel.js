  ...
  submit(submission) {
    if (this.stimulusPlaybackHasEnded && !this.feedbackHasBeenPlayed) {
      ...
      if (submission.choice === Choice.first)
        this.observer.notifyThatFirstChoiceIsSelected();
      else if (submission.choice === Choice.second)
        this.observer.notifyThatSecondChoiceIsSelected();
    }
  }
...
export class TaskModelWithoutFeedback {
  ...
  submit(submission) {
    if (this.stimulusPlaybackHasEnded && !this.notifiedTaskEnding) {
      this.observer.notifyThatTaskIsReadyToEnd();
      this.notifiedTaskEnding = true;
      this.result.choice = choiceName(submission.choice);
      this.result.word = this.words.get(submission.choice);
      this.result.correct =
        this.words.get(submission.choice) === this.correctWord ? "yes" : "no";
